...
def total_image_predict(ori_image_path):
    ...
    for h in range(h_step):
        for w in range(w_step):
            ...
            tmp[(h - 1) * over_lap + (big_w if h > 0 else over_lap):(h - 1) * over_lap + (
                big_w if h > 0 else over_lap) + dh,
            (w - 1) * over_lap + (big_w if w > 0 else over_lap):(w - 1) * over_lap + (
                big_w if w > 0 else over_lap) + dw] \
                = predict_list[count_temp][small_w * int(h > 0):dh + small_w * int(h > 0),
                  small_w * int(w > 0):dw + small_w * int(w > 0)]
            count_temp += 1
        if h < h_step - 1:
            tmp[(h - 1) * over_lap + (big_w if h > 0 else over_lap):(h - 1) * over_lap + (
                big_w if h > 0 else over_lap) + dh, -w_rest:] \
                = predict_list[count_temp][small_w if h > 0 else 0:(small_w if h > 0 else 0) + dh, -w_rest:]
            count_temp += 1

    for w in range(w_step):
        if w < w_step - 1:
            tmp[-h_rest:, (w - 1) * over_lap + (big_w if h > 0 else over_lap):(w - 1) * over_lap + (
                big_w if h > 0 else over_lap) + dw] \
