...
def dilated_conv2D_layer(inputs, num_outputs, name='weight', kernel_size=3, activation_fn=None, rate=2, padding='SAME'):
    ...
    kernel = tf.Variable(tf.random.truncated_normal([kernel_size, kernel_size, in_channels, num_outputs], stddev=0.1), name=name)
    inputs = tf.nn.conv2d(inputs, kernel, strides=[1, 1, 1, 1], padding=padding, dilations=[1, rate, rate, 1])
